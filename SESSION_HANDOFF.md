...
- `scripts/compile_culture_source_summary.py`가 normalized culture items 전체를 소스별로 집계해 `app/static/data/culture_source_summary.json`을 만들고, 랜딩의 새 Culture Source Pulse 카드가 각 입력 소스의 행수와 최신 수집 시점을 브랜드/CTA 앞에서 강조하도록 했습니다.
